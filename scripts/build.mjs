...
function richText(value) {
  ...
  while ((match = pattern.exec(text)) !== null) {
    ...
    html += internalLinkRoutes.has(href) || internalLinkRoutes.has(basePath)
...
function imageAlt(image) {
  if (!image || !Object.prototype.hasOwnProperty.call(image, 'alt')) {
    return 'Clearwater Dentist';
  }
  return image.alt;
}
function imageTag(image, cls, eager) {
  if (!image || !image.src) return '';
  return '<img class="' + attr(cls || '') + '" src="' + attr(image.src) + '" alt="' + attr(imageAlt(image)) + '" loading="' + (eager ? 'eager' : 'lazy') + '" decoding="async">';
}
...
function compareCard(pair, index) {
  ...
  return '<article class="compare-card cw-reveal" data-cw-compare-id="' + attr(caseId) + '" data-cw-compare-index="' + index + '"' + cwRevealAttr(dir, index * 100) + '><div class="compare-slider" style="--position:50%"><img class="compare-img compare-before"' + cwEdit('/', 'compare-' + index + '-before', 'Before image ' + (index + 1), 'img') + ' src="' + attr(pair.before) + '" alt="' + attr(pair.name + ' before') + '" loading="lazy" decoding="async"><div class="compare-after-wrap"><img class="compare-img compare-after"' + cwEdit('/', 'compare-' + index + '-after', 'After image ' + (index + 1), 'img') + ' src="' + attr(pair.after) + '" alt="' + attr(pair.name + ' after') + '" loading="lazy" decoding="async"></div><input class="compare-range" type="range" min="0" max="100" value="50" aria-label="Reveal before and after image"><span class="compare-handle" aria-hidden="true"></span><span class="compare-label compare-label-before">Before</span><span class="compare-label compare-label-after">After</span></div><h3' + cwEdit('/', 'compare-' + index + '-title', 'Case title ' + (index + 1)) + '>' + e(pair.name) + '</h3></article>';
}
function beforeAfterSection(limit) {
  ...
  const parallaxBgMobile = site.assets.beforeAfterParallaxMobile || parallaxBg.replace(/-1280w(?=\.webp$)/i, '-768w');
...
function homeLeadForm() {
  return '<section id="homeLeadForm" class="home-lead-panel"><div class="home-lead-panel__enter"><div><p class="eyebrow">New Patients</p><h2>Request an appointment without leaving the page.</h2><p>Tell us how to reach you and what brought you in. Our team follows up during office hours with scheduling options, new-patient details, and answers to common first-visit questions.</p><div class="mini-actions"><a href="/contact-us">Contact page</a><a href="/financing">Financing options</a></div></div><form class="mini-form" action="/contact-us" method="get"><div class="mini-form__split"><div class="mini-form__row"><label for="home-lead-name">Name</label><input id="home-lead-name" name="name" autocomplete="name" required></div><div class="mini-form__row"><label for="home-lead-phone">Phone</label><input id="home-lead-phone" name="phone" type="tel" autocomplete="tel" required></div></div><label class="mini-form__full-label" for="home-lead-reason">What do you need?</label><select id="home-lead-reason" name="reason"><option>New patient visit</option><option>Emergency appointment</option><option>Cosmetic consultation</option><option>Dental implants</option></select><button class="btn primary" type="submit">Start Request</button></form></div></section>';
}
...
function whySection() {
  ...
  const whyBgMobile = site.assets.whyBandParallaxMobile || whyBg.replace(/-1280w(?=\.webp$)/i, '-768w');
...
function contactMapSection() {
  ...
  return '<section class="contact-map-band cw-google-trust" aria-label="Office location"><div class="cw-google-trust"><div class="cw-trust-map-row cw-trust-map-row--contact" data-cw-map-row><div class="map-card"><div class="map-frame-wrap"><iframe src="' + attr(embed) + '" width="600" height="450" style="border:0;" allowfullscreen loading="lazy" referrerpolicy="no-referrer-when-downgrade" title="Clearwater Dentist on Google Maps"></iframe><div class="map-place-badge" aria-label="Clearwater Dentist ' + attr(rating) + ' stars, ' + attr(count) + ' Google reviews"><strong class="map-place-badge__name">Clearwater Dentist</strong><span class="map-place-badge__score"><span class="map-place-badge__stars" aria-hidden="true">★★★★★</span><span>' + rating + ' · ' + count + ' reviews</span></span></div></div><p class="map-card__address">' + addressLine + '</p></div><figure class="cw-trust-office-photo" data-cw-map-photo aria-hidden="false"><img src="' + attr(officePhoto) + '" alt="Clearwater Dentist office exterior at 1700 N McMullen Booth Rd" width="640" height="480" loading="lazy" decoding="async"></figure></div></div></section>';
}
...
function renderHome(page) {
  ...
  const videoMobile = site.assets.heroVideoMobile || video;
  const poster = site.assets.heroPoster || site.assets.office;
  const rk = page.route;
  const heroMedia = video
